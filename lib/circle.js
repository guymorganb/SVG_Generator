...
class Circle extends shapes {
    ...
    // renders the circle
    render() {
        return `<svg xmlns="${this.xmlns}" width="${this.width}" height="${this.height}">
                    <circle fill="${this.#color}" cx="${this.cx}" cy="${this.cy}" r="${this.r}"/>
                    <text x="${this.textx}" y="${this.texty}" text-anchor="${this.textAnchor}" dominant-baseline="${this.dominantBaseline}" fill="${this.textColor}" font-size="${this.fontSize}">${this.innerText}</text>
                </svg>`;
    }
}
